...
const originalWorkspacesFlag = Flag.OCTO_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  OCTO_AUTH_CONTENT: process.env.OCTO_AUTH_CONTENT,
...
afterEach(async () => {
  ...
  Flag.OCTO_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
describe("workspace-old CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      process.env.OCTO_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      ...
      expect(JSON.parse(recorded.calls.create[0].env.OCTO_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.OCTO_WORKSPACE_ID).toBe(workspaceID)
      expect(recorded.calls.create[0].env.OCTO_EXPERIMENTAL_WORKSPACES).toBe("true")
...
describe("workspace-old sync state", () => {
  test("startWorkspaceSyncing is disabled by the experimental workspace flag", async () => {
    await withInstance(async (dir) => {
      Flag.OCTO_EXPERIMENTAL_WORKSPACES = false
