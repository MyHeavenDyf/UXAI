...
function createBadge() {
  ...
  badge.innerHTML = `
    ...
    <div id="${COPY_SOURCE_ID}">选中区域</div>
  `
...
export function installDomPicker(options = {}) {
  ...
  copySource.addEventListener('click', async (event) => {
    event.preventDefault()
    event.stopPropagation()
    const location = badge.dataset.source || activeLocation || activeElement?.getAttribute(ATTRIBUTE_NAME) || activeElement?.getAttribute(PICKER_ID_ATTR)
    if (!location) {
      return
    }
    ...
    if (navigator.clipboard?.writeText) {
      try {
        ...
        window.parent.postMessage({ type: 'DOM_PICKER_COPY', domPickerId: location, tagName: activeElement.tagName.toLowerCase(), rect: { top: rect.top, left: rect.left, width: rect.width, height: rect.height }, clickX: event.clientX, clickY: event.clientY }, '*')
      } catch {
      }
    } else {
