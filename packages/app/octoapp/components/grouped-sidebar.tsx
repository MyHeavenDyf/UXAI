  ...
  const handleRenameGroup = (group: MakeGroup) => {
    dialog.show(() => (
      <DialogCreateGroup
        ...
        onCreate={(name) => { tracker.interaction({ module: props.trackerModule ?? "session", name: "rename-group" }); return renameGroup(group.id, name) }}
      />
    ))
  }
      ...
      beforeSection={(api) => {
        ...
        <>
          <SidebarSectionHeader
            ...
            onAction={() => dialog.show(() => <DialogCreateGroup existingNames={groups.map(g => g.name)} onCreate={(name) => { tracker.interaction({ module: props.trackerModule ?? "session", name: "create-group" }); return addGroup(name) }} />) }
          />
