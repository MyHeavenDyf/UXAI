...
export function modelsApiSource() {
  if (REMOTE_MODELS_ONLY) return "http"
...
export function modelsApiHeaders() {
  ...
  const w3Api = REMOTE_MODELS_ONLY
    ? undefined
    : source === "http"
      ? latestModelsApi?.w3?.api?.trim() || cachedW3Api(url)
      : undefined
  return {
