...
你的文字回复必须且仅是：一个用 \<node-dsl\> 标签包裹的完整 JSON 对象，形如 \<node-dsl\>{...完整 JSON...}\</node-dsl\>。回复第一个字符是 \<node-dsl\> 的 \<，最后一个字符是 \</node-dsl\> 的 \>。标签内是且仅是合法 JSON（以 { 开头、} 结尾），标签外不得有任何文字；不使用 markdown 代码块、不使用 \<artifact\> 或其它标签。
...
- 元素间距的语义用 gap 表达；但每个节点的 rect 坐标（x/y/w/h）仍必须精确反映它在页面中的真实位置——**渲染直接按 rect 坐标定位，坐标估算不准就会错位、左右不对称、不居中**。不要以为"用了 flex 就不用算坐标"。
- 容器宽度优先继承父容器（rect.w 与父级相同），高度按内容自适应或设定合理值

#### 水平居中与对称留白（块级元素务必精确居中）

rect 是页面绝对坐标，块级元素的水平位置完全由 rect.x 决定。按下列公式**精确计算 rect.x**（结果四舍五入取整）：

- **水平居中的块**（卡片、居中按钮、Logo、插画等）：\`x = 父.x + (父.w − 自身.w) / 2\`。例：根容器 x=0、w=375，卡片 w=335 → x = (375−335)/2 = 20。
- **左右对称留白的内容区**：左右边距相等（移动端 20，桌面端 40）。\`x = 父.x + 边距\`，\`w = 父.w − 2×边距\`。例：根 375、边距 20 → x=20、w=335。
- **满宽/贴边元素**（导航栏、标签栏、分割线、整宽 banner）：\`x = 父.x\`，\`w = 父.w\`。
- 子元素必须落在父容器内：\`父.x ≤ 子.x\` 且 \`子.x + 子.w ≤ 父.x + 父.w\`；同一父下同类块级子元素的 x（或左右边距）应一致，避免参差。

同时让 flex 属性与坐标自洽（两者都要表达居中，不能只写其一）：水平居中的 column 容器写 \`alignItems: "center"\`，水平居中的 row 容器写 \`justifyContent: "center"\`。
...
1. **回复只能是 \<node-dsl\> 包裹的 JSON** — 形如 \<node-dsl\>{...}\</node-dsl\>：回复以 \<node-dsl\> 开头、以 \</node-dsl\> 结尾，标签内是且仅是合法 JSON（{ 开头 } 结尾），标签必须成对闭合。
2. **禁止任何标签内外的多余文字** — 标签外不输出任何字符；标签内只有 JSON，不出现"好的"、"以下是"、分析说明、总结、页面描述等。
3. **禁止 markdown 代码块** — 不使用 \`\`\`json 或任何代码围栏（\<node-dsl\> 标签内直接是裸 JSON）。
4. **禁止 \<artifact\> 或其它包裹标签** — 除 \<node-dsl\> 外不使用任何 XML/HTML 标签。
...
**发送前自检**：回复是否以 \<node-dsl\> 开头、以 \</node-dsl\> 结尾且标签成对闭合？标签内是否以 { 开头、} 结尾且为合法 JSON？标签内外是否有任何非 JSON 文字、markdown 或 HTML/CSS/JS 代码？如有，修正后再发送。
`
