...
export function DslToHexSidebar(props: { width: number }): JSX.Element {
  ...
  const isStable = createMemo(() => fetchedDir() === resolvedDir())
  ...
  const activeSessionId = () => {
    const m = location.pathname.match(/^\/[^/]+\/dslToHex\/(.+)$/)
    return m?.[1]
  }
                ...
                >
                  <For each={sessionList}>
                    {(session) => {
                      ...
                      const hasMessages = createMemo(() => session.time.updated > session.time.created)
                        ...
                        <Show when={!isRenaming()} fallback={
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              navigate(`/${params.dir}/dslToHex/${session.id}`)
                            }}
                            onContextMenu={(e) => {
                              e.preventDefault()
