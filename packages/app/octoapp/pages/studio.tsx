import "@/pages/cowork/octo-tokens.css"
...
import { AttachmentBar, type Attachment } from "@/pages/cowork/components/attachment-bar"
import { InsightTurn, type OutputCard } from "@/pages/cowork/components/insight-turn"
import { ResultViewer } from "@/pages/cowork/components/result-viewer/index"
import { createTabStore } from "@/pages/cowork/components/result-viewer/tab-store"
...
export default function StudioPage() {
  ...
  const resolvedDirectory = createMemo(() => projectDir() ?? null)
    ...
    on(
      ...
      (id) => {
        ...
        globalSDK.client.session.messages({ sessionID: id })
          ...
          .catch((err) => {
            console.error("[StudioPage] messages load failed", err)
          })
      },
    ),
  )
  ...
  async function createAndNavigate(): Promise<string | undefined> {
    ...
    try {
      ...
      console.error("[StudioPage] session.create failed", err)
    } finally {
      setSending(false)
    }
    return undefined
  ...
  async function sendMessage(sessionId: string, text: string) {
    ...
    try {
      ...
      console.error("[StudioPage] prompt failed", err)
    } finally {
      setSending(false)
    }
  }
...
function StudioResultEmpty(): JSX.Element {
  return (
