...
const sessionMessagesLoaded = createMemo(() => {
  ...
  async function handleOpenResult(card: OutputCard) {
    const fromAttachment = card.fromAttachment ?? true  // 默认会话区入口
  ...
  function handleOpenLocalFile(filePath: string, sessionArea = false) {
    ...
    // 会话区:仅 图片/视频/音频/PDF → 右侧预览;非会话区:仅拦截 Office,其余均可预览
    const blocked = sessionArea
      ? !["image", "video", "audio", "pdf"].includes(type)
      : ["ppt", "pptx", "pps", "ppsx", "xls", "xlsx", "xlsm", "doc", "docx"].includes(fileExt)
    ...
    if (blocked) {
      ...
      tracker.interaction({ module: "design", name: "preview-local-file", extend: JSON.stringify({ type: sessionArea ? "unavailable" : "office-unavailable", ext: fileExt }) })
      return
    }
