...
export function ColorPicker(props: {
  ...
  function updatePos() {
    ...
    const panelH = 330
    setPopupPos({
      x: Math.max(4, Math.min(rect.left - panelW - 6, window.innerWidth - panelW - 4)),
      y: Math.max(4, Math.min(rect.top - 72, window.innerHeight - panelH - 4)),
    })
    requestAnimationFrame(() => {
      if (!popupRef) return
      const actualH = popupRef.getBoundingClientRect().height
      const clampedY = Math.max(4, Math.min(rect.top - 72, window.innerHeight - actualH - 4))
      setPopupPos({
        x: Math.max(4, Math.min(rect.left - panelW - 6, window.innerWidth - panelW - 4)),
        y: clampedY,
      })
