...
/** 从 A2UI 文档构建同层元素 siblingMap（用于 DRAG_MODE 拖拽换序） */
...
async function loadA2uiData(ctx: SubtypeHandlerContext): Promise<unknown | null> {
  ...
  const jsonStr = text.replace(/^window\.__A2UI_DATA__\s*=\s*/, "").replace(/;\s*$/, "")
  try {
...
async function persistA2uiData(filePath: string) {
  ...
  await getDesktopApi()?.writeFileBuffer?.(`${dir}/data.js`, buffer)
}
...
export function closePrototypePanels() {
  sendToPrototypeIframe({ type: "DOM_PICKER_UNFREEZE" })
...
/** 应用一次属性编辑到当前 prototype 的 A2UI JSON 并 A2UI_UPDATE 回推重渲染。
...
/** 拖拽换序：在 A2UI JSON 中重排同级 children（静态 string[] 或循环 {path,componentId}）并 A2UI_UPDATE 回推 */
...
export default {
  ...
  async handleLocalEdit(ctx) {
    const next = !editing
    ctx.postMessageToIframe?.({ type: "DOM_PICKER_TOGGLE", active: next })
    if (!next) {
      ctx.postMessageToIframe?.({ type: "DRAG_MODE", enabled: false })
    ...
    ctx.postMessageToIframe?.({ type: "DRAG_MODE", enabled: true, siblingMap })
    if (!messageHandler) {
      messageHandler = (e: MessageEvent) => {
        ...
        if (!t.startsWith("DOM_PICKER") && t !== "DRAG_REORDER" && t !== "A2UI_READY") return
        if (t === "DOM_PICKER_CONTEXT_MENU") {
          const iframe = currentCtx?.iframeElementGetter?.()
          const rect = iframe?.getBoundingClientRect()
          const scale = rect && iframe ? rect.width / (iframe.offsetWidth || 1) : 1
          dispatchPrototypeCtxMenu({
            x: rect ? rect.left + (d.clickX ?? 0) * scale : 0,
            y: rect ? rect.top + (d.clickY ?? 0) * scale : 0,
            id: String(d.id ?? ""),
