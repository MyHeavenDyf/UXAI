...
function wrapComponentsPrompt(filePath: string, body: string): string {
  const previewRoot = filePath.replace(/[/\\][^/\\]+$/, '')
  return [
    `预览页面源码根目录：${previewRoot}`,
    '该预览由 ict-component-creator skill 的 build.mjs 编译生成，index.components.html 是编译产物，不可直接编辑。',
    '真正的源码在 components/{组件名}/index.jsx、index.css 以及 demo.jsx 中。',
    '请按下方信息定位源码、应用变更，然后重新编译并验证。',
    '',
    body,
    '',
    '操作要求：',
    '1. 按类名在 components/*/index.jsx（结构、文本）和 index.css（样式）中定位对应代码；展示区布局改 demo.jsx',
    '2. 只改源码文件，不要改 index.components.html',
    `3. 重新编译：node scripts/build.mjs --dir "${previewRoot}"`,
    `4. 验证：node scripts/verify-build.mjs --dir "${previewRoot}"`,
    '5. build 产物 index.components.html 更新后预览会自动刷新',
  ].join('\n')
}
...
export default {
  ...
  modelEditConfig: componentsModelEditConfig,
