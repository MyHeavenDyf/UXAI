...
export function buildChipTemplate(sel: McpSelection, typedText: string): string {
  ...
  return [
    ...
    ``,
    `后续查询仪式(适用于本会话之后的轮次):服务端会在后台自动查询已提交任务并把完成结果持久化回原会话,不要求用户停留在当前页面。用户明确说「查询任务 X」「好了吗」「看看进度」时仍调用 get_task_result 做即时刷新;不得凭记忆编造状态。已完成且文件卡片已经出现时可直接说明结果已返回,无需重复提交任务。`,
    ``,
