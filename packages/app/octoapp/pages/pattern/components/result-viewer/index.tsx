...
import { A2UIJsonViewer } from "../a2ui-json-viewer"
...
function JsonFallbackRenderer(props: { content: string }): JSX.Element {
...
export function ResultViewer(props: {
  ...
  const isA2UIDoc = createMemo(() => {
    const tab = activeTab()
    if (!tab || tab.type !== "json") return false
    return detectA2UIJson(tab.content) !== null
  })
            ...
            <div class="flex flex-col flex-1 min-h-0 overflow-hidden">
              ...
              <div class="flex-1 min-h-0 overflow-hidden">
                ...
                >
                  ...
                  <Match when={tab().type === "json" && isA2UIDoc()}>
                    <A2UIJsonViewer content={tab().content} />
                  </Match>
                  <Match when={tab().type === "json"}>
                    <JsonFallbackRenderer content={tab().content} />
                  </Match>
...
function ResultViewerEmpty(): JSX.Element {
  return (
    <div class="flex flex-col items-center justify-center h-full gap-2 text-center px-8">
      ...
      <div class="text-[13px]" style={{ color: "var(--octo-text-secondary)" }}>Pattern 产出将在这里展示</div>
      <div class="text-[12px]" style={{ color: "var(--octo-text-disabled)" }}>支持 A2UI JSON、HTML、SVG、表格等多种格式</div>
    </div>
  )
}
