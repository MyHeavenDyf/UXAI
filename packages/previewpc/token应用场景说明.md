...
- 所有图表组件默认内置色组[#2070F3, #62B42E, #715AFB, #2CB8C9, #F69E39, #5CA2E9]，只有遇到极强的需要状态色时，才可以使用 "error": "#E02128","success": "#09AA71", "critical": "#F4840C",  "warning": "#FCC800"替换对应的系列颜色，优先不设置color，颜色只能从这里面选取，不能使用其他颜色！
  - **Constraints:**
    - color数组的length必须大于或等于图表数据的系列数量。BAD：数据有2个系列，但是color:["#2070F3"] - 这会导致该图表所有系列都是相同颜色！