 ...
 */
function parseCssToChunks(css: string, useVar: boolean = false): ParsedChunk[] {
  ...
  while (i < s.length) {
    ...
    if (stop === "{") {
      ...
      frames.push(
        sel.includes("&")
          ? { kind: "pseudo", pseudo: sel, localVars: new Map(propertyVars) }
...
export function convertTailwindToCSSGrouped(className: string, useVar: boolean = false): GroupedCSS {
  ...
  for (const css of designSystem.candidatesToCss(classes)) {
    if (!css) continue
    for (const chunk of parseCssToChunks(css, useVar)) {
...
export function generateLessContent(rules: LessRule[]): string {
  ...
  for (const rule of rules) {
    if (rule.declarations.length > 0) {
      inner.push(`${rule.selector} {`)
      for (const decl of rule.declarations) inner.push(`  ${decl.prop}: ${decl.value};`)
      inner.push('}')
      inner.push('')
    }
    for (const v of rule.variants ?? []) {
      const indent = v.mediaQueries.length > 0 ? '  ' : ''
      if (v.mediaQueries.length > 0) inner.push(`@media ${v.mediaQueries.join(' and ')} {`)
      inner.push(`${indent}${v.selector} {`)
      for (const decl of v.declarations) inner.push(`${indent}  ${decl.prop}: ${decl.value};`)
      inner.push(`${indent}}`)
      if (v.mediaQueries.length > 0) inner.push('}')
      inner.push('')
    }
  }
  while (inner.length > 0 && inner[inner.length - 1] === '') inner.pop()
  // 不包 @layer utilities——CSS Modules (postcss-modules) 经 LESS 预编译后
  // 可能不从 @layer 块内提取 class exports，导致 styles.<id> 为 undefined。
  // 裸规则 unlayered 优先级高于 tailwind-base.css 的 layered 规则，
  // 元素级样式自然压过 preflight / 泛用 utility，符合预期。
  const lines: string[] = ['// Auto-generated by a2ui-transformer', '']
