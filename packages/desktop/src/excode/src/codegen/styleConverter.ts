 ...
 *   3. generateGlobalLess() → src/styles/variables.less 骨架
 *
...
export class StyleConverter {
  ...
  convertPage(
    ...
    // 全局 LESS 变量骨架（一期空骨架，所有页面共用同一份）
    const globalLess = generateGlobalLess()
...
function generateGlobalLess(): string {
  // 一期空骨架，CSS Modules 全局 token 由各 module 文件各自覆盖
  return [
    '// Auto-generated global LESS variables',
    '// (extend with design tokens when needed)',
    '',
  ].join('\n')
}
