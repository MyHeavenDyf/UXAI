...
任务记录中正在执行的任务会显示“停止”按钮。停止请求由主控转发到实际构建节点：

- macOS ARM64 和 macOS x64：打包脚本在独立进程组中运行，停止时先向整个进程组发送 `SIGINT`，效果等同于终端按 `Ctrl+C`；5 秒后仍未退出才强制结束。
- Windows x64：使用 `taskkill /T /F` 结束打包脚本及其完整子进程树，避免残留 Electron、Bun 或打包工具进程。
- 如果任务仍处于切分支、拉代码或刷新脚本缓存阶段，服务会记录停止请求，并在最近的安全检查点结束。
- 停止后仍会执行 Git 改动清理，任务状态记录为 `stopped`（页面显示“已停止”），随后队列自动执行下一个任务。
