...
Mac 任务的 `.dmg` 产物同时提供普通浏览器下载和“终端下载（复制命令）”按钮。点击终端下载后，页面会把完整 `curl` 命令复制到剪贴板；用户只需打开终端、粘贴并回车。

命令会把安装包保存到 `~/Downloads`，先比较远端与本地文件长度：本地较小时断点续传、大小相同时直接打开、本地较大或长度异常时覆盖下载。完成后只清除该 DMG 的浏览器隔离属性，并自动通过 `open` 挂载。

```bash
output="$HOME/Downloads/安装包.dmg"
url="安装包地址"
# 自动比较长度、续传或覆盖，完成后执行：
xattr -d com.apple.quarantine "$output"
open "$output"
```

这种方式继续使用现有 HTTP 服务，不需要配置 SMB、文件共享或额外端口。重复点击同一安装包时不会因为文件已经完整而触发 HTTP 416；此前由浏览器下载的同名完整文件也会在打开前移除隔离属性。
